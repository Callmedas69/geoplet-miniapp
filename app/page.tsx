...
export default function Home() {
      ...
      if (fidIsMinted) {
        ...
        try {
          ...
          if (data.ownedNfts && data.ownedNfts.length > 0) {
            ...
            console.log(
              "[LOAD-GEN] FID minted but NFT not owned (transferred)"
            );
  ...
  useEffect(() => {
    ...
    const generationDecided = hasAutoGenerated || generatedImage !== null;

    if (hasWallet && hasWarpletData && storageCheckComplete && generationDecided) {
  ...
  }, [address, nft, fid, isLoadingStorage, hasAutoGenerated, generatedImage]);
    ...
    const autoGenerate = async () => {
      ...
      try {
        ...
        if (fidIsMinted) {
          ...
          console.log(
            "[AUTO-GEN] FID already minted, fetching minted Geoplet from Alchemy..."
          );
          setIsMinted(true);
          ...
          try {
            ...
            if (data.ownedNfts && data.ownedNfts.length > 0) {
              ...
              console.log(
                "[AUTO-GEN] FID minted but NFT not owned by current wallet (transferred)"
              );
              toast.info("You already minted with this FID");
            }
          } catch (alchemyError) {
            console.error(
              "[AUTO-GEN] Failed to fetch minted Geoplet from Alchemy:",
              alchemyError
            );
        ...
        let friendlyError =
          "😅 Auto-gen unavailable. Try Regenerate or refresh!";
